...
document.addEventListener('DOMContentLoaded', () => {
    ...
    resetButton.addEventListener('click', () => {
        if (confirm('Êtes-vous sûr de vouloir réinitialiser le jeu ? Tout progrès sera perdu.')) {
            ...
            // Reset points
            if (window.gamePoints) {
                // Reset points to 0
                points = 0;
                window.gamePoints.updatePointsDisplay();
            }
...
function startAutoClicker() {
    ...
    autoClickInterval = setInterval(() => {
        if (autoClicksPerSecond > 0) {
            if (window.gamePoints) {
                window.gamePoints.addPoints(Math.floor(autoClicksPerSecond / 10));
            }
        }
...
function buyMob(mobType, clicksPerSecond, cost) {
    ...
    if (window.gamePoints && typeof window.gamePoints.subtractPoints === 'function') {
        if (window.gamePoints.subtractPoints(cost)) {
            ...
            // Mettre à jour l'apparence du mob
            const mobImages = document.querySelectorAll('.img-village-bonus');
            mobImages.forEach(img => {
                if (img.src.includes(mobType) || (img.alt && img.alt.toLowerCase().includes(mobType))) {
